...
import { BattleReportsList } from "../components/BattleReportsList.js";
import { FleetMissionPanel, FleetMissionsList } from "../components/FleetMissionPanel.js";
import { OrbitIntrudersList } from "../components/OrbitIntrudersInfo.js";
...
export function FleetsPage() {
  ...
  const { catalog, state, loading, refresh } = useGame();
  ...
  const selectionTotals = useMemo(() => {
    ...
    for (const q of Object.values(selectionPayload)) {
    ...
    <div className="fleet-page">
      ...
      {p && (
        ...
        <p className="page-lead">
          ...
          . Выберите корабли и укажите цель <strong>[A:S:P]</strong> с орбитой.
      ...
      {rows.length > 0 && (
        ...
        <>
          ...
          <FleetMissionPanel
            ...
            selectionShips={selectionTotals.ships}

            onSent={() => void onFleetSent()}
      ...
      <OrbitIntrudersList
        intruders={state?.orbitIntruders ?? []}
        unitNames={new Map(catalog.units.map((u) => [u.id, u.name]))}
      />

      <BattleReportsList reports={state?.battleReports ?? []} />
